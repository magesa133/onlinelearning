    ...
    try:
        ...
        if not enrollments:
            debug_info = {
                ...
                'enrolled_classrooms': None,
                'total_published_quizzes': Quiz.query.filter_by(status=QuizStatus.PUBLISHED.value).count(),
                'time_zone': str(tz)
            }
        ...
        # Get published quizzes for enrolled classrooms
        quizzes = Quiz.query.filter(
            Quiz.classroom_id.in_(class_ids),
            Quiz.status == QuizStatus.PUBLISHED.value  # Use status enum instead of published
        ).options(
        ...
        for quiz in quizzes:
            if quiz.due_date:
                # Ensure quiz.due_date is timezone-aware
                if quiz.due_date.tzinfo is None:
                    quiz_due_date = local_timezone.localize(quiz.due_date).astimezone(tz)
                else:
                    quiz_due_date = quiz.due_date.astimezone(tz)
        ...
        # Get other dashboard data (assignments, activities, etc.)
        ...
        sessions = get_student_sessions(class_ids, now)
        ...
        # Get only completed quiz attempts
        recent_attempts = QuizAttempt.query.filter(
            QuizAttempt.student_id == student.id,
            QuizAttempt.completed_at.isnot(None)  # Filter out incomplete attempts
        ).options(
...
def student_view_quiz(quiz_id):
    """View quiz details and attempt history"""
...
# Helper functions
def handle_quiz_submission(attempt, quiz):
    """Process quiz submission with error handling"""
    try:
        # Process all answers
        answers = process_answers(request.form, quiz.questions)
        
        # Save answers
        for q_id, answer in answers.items():
            save_answer(attempt, q_id, answer, quiz)
...
def process_answers(form_data, questions):
    """Extract and validate answers from form data"""
    answers = {}
    for q in questions:
        answer_key = f"question_{q.id}"
        if answer_key in form_data:
            answers[q.id] = form_data[answer_key]
    return answers

def save_answer(attempt, question_id, selected_answer, quiz):
    """Save or update an answer"""
    existing = next((a for a in attempt.answers if a.question_id == question_id), None)
    
    if existing:
        existing.selected_answer = selected_answer
        existing.is_correct = (selected_answer == 
            next(q.correct_option for q in quiz.questions if q.id == question_id))
...
def get_current_question(attempt, quiz, requested_id=None):
    """Determine which question to show next"""
    answered_ids = {a.question_id for a in attempt.answers}
    
    if requested_id:
        # Return requested question if valid
        question = next((q for q in quiz.questions if q.id == requested_id), None)
        if question:
            return question
    
    # Return first unanswered question
    return next((q for q in quiz.questions if q.id not in answered_ids), None)

def get_question_number(attempt, question):
    """Get the position of the current question"""
    now = datetime.now(timezone.utc)
    answered_before = sum(1 for a in attempt.answers 
                         if a.answered_at.replace(tzinfo=timezone.utc) < now)
    return answered_before + 1

def calculate_time_remaining(attempt, quiz):
...
def complete_attempt(attempt):
    """Finalize quiz attempt and show results"""
    attempt.completed_at = datetime.now(timezone.utc)
    started_at = attempt.started_at.replace(tzinfo=timezone.utc) if attempt.started_at.tzinfo is None else attempt.started_at
    attempt.time_spent = (attempt.completed_at - started_at).total_seconds()
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error(f"Failed to complete attempt: {str(e)}")
        flash('Error completing quiz. Please try again.', 'danger')
        return redirect(url_for('student_take_quiz', attempt_id=attempt.id))
...
def student_take_quiz(attempt_id):
    """Take or continue a quiz attempt"""
    ...
    if quiz.due_date and quiz.due_date < datetime.now(timezone.utc):
    ...
    if request.method == 'POST':
        ...
        # Check if submitting final answer
    ...
    if question_id:
        current_question = next((q for q in quiz.questions if q.id == question_id), None)
    else:
        current_question = quiz.questions[0] if quiz.questions else None
    ...
    time_remaining = None
    if quiz.time_limit:
        time_elapsed = (datetime.now(timezone.utc) - attempt.started_at).total_seconds() / 60
    ...
    return render_template('student/quizzes/take.html',
        ...
        question_num=quiz.questions.index(current_question) + 1,
